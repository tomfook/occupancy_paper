...
\begin{pmatrix}
  ...
  p_1 & 0 & \vdots & 0\\
\end{pmatrix}
\right)=:\check\pi(I_{n-1}-U_{n-1}-D_{n-1}+P_{n-1})
$$
ただし
$$
\check p=(p_1,p_2,\ldots,p_{n-1})\\
\check \pi=(\pi_1,\pi_2,\ldots,\pi_{n-1})\\
P_{n-1} = \begin{pmatrix} p_1 & p_2 & \ldots & p_{n-1}\\ p_1 & p_2 & \ldots & p_{n-1} \\ \vdots & \vdots & \ddots & \vdots \\ p_1 & p_2 & \ldots & p_{n-1}\end{pmatrix}
$$
である。
よって右辺の$I_{n-1}-U_{n-1}-D_{n-1}+P_{n-1}$の逆行列を求めて、両辺に左からかけることで定常状態$\pi$を得ることができることがわかった。
...
来場者数の分布の連続に拡張する必要があるが、正で有界な値域を取る一般の分布では解析が難しそうなので、何か仮定を置く必要がありそうだが、どのような仮定を採用するべきだろうか？
...
$p=p_1=1-p_2$に関してグラフを書くと次の図が得られる。
...
関数を$p$に関して微分し極値を求めると$p=0.46$のときに最小値`r 3.81966` (席あたりでは `r 3.81966/4`)を取る。


### 眼の前のチャレンジ
- 列あたり2⃣席の場合は列数を1から2に増やすと席数あたり期待Occupancyが増えて極値が$p$に関して左側にシフトした。列数をさらに増やしても同様の傾向は主張できるだろうか？
