...
## Introduction
One of the most important issues for theme park operations is to raise guest counts of attractions.
Since guest counts is decomposed by the dispatch count of vehicles and seat occupancy, we usually focus on them.
...
For ride attractions, attraction crews form a group of arrival guests to match the vehicle's capacity.
The operation is called a grouping operation and the crew is called a grouper.
The grouping operation is the most important factor in deciding seat occupancy.
Guests arrive at an attraction as a group of friends.
A grouper should form a dispatch group so as not to break the friend groups.
...
Grouping operation is to gather people to have a seat in the same line.
...
Grouper asks for the size of the group and selects a line to guide.
The grouping strategy is to select an optimal line when the size of group comes.
