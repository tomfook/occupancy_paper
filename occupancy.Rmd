...
## Introduction
One of the most important issues for theme park operation is raising guestcounts at attractions.
...
A grouper guides earlier-arriving guests first.
...
This means that seats are arranged in row, with one or more rows lined up.
The grouping operation involves gathering people to sit in the same row.
Of course, friend groups should remain together in the same group and sit next to each other.
...
Then the expectation of seat occupancies is calculated from a transition probability $P[x_i=k]=p_k$.
and how to derive the probability having each state $P[a_i=n]$ from $p_k$ because the expectation value can be easily calculated by $p_k$ and $P[a_i=n]$.
