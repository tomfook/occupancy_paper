...
If we obtain the value, it could be possible to clarify the efficiency of grouping operation and whether seat occupancy can improve or not.
In addition we will be able to judge which operations can achieve more seat occupnacy.

This upper bound can be probability variable because arrivals of guest should be probabilistic.
Thus the expected value of 

### Modeling
To evaluate the upper bound, we set some operational situations as probabilistic model.
Thus we will obtain the upper bound as expectation value.  
Let

- the size of $i$th arrival group be probability variable $x_i$.
- the number of seats filled with persons be a probability variable $a_i$ after the $i$the group have seats.
