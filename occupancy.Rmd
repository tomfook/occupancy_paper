...
We revealed that seat occupancy is derivable by statistics of group size, queue formation, the number of seats and grouping strategy.  
This framework will help us to optimize queue formation and grouping strategy.
Because the model is general, that is, the model is not dependent on any actual attraction, our framework can be applied to every attractions.

## Modeling and Framework
We consider about ride attractions.
We want to find out how much seat occupancy we can achieve.
If we have theoretical seat occupancy, we can compare actual value with it and evaluate its efficiency.
Then we can identify which attractions can be more efficient.
Or if we have other options for grouping strategy, we can judge if we change strategy or not.

Mathematical setting

- the size of $i$th arrival group is $x_i$
- the probability that a group sized $k$ arrives is stationary and $p[x_i=k]=p_k$ 
- After the $i$th group have seats, the number of seats filled with persons is $a_i$

To evaluate the expectation value of seat occupancy, we apply **Markov chain model** to this situation.
Markov chain model is a stochastic model using state and transition.

We set $a_i$ as state after $i$ times transitions.
