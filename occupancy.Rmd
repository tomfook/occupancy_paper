...
### Model
To evaluate the upper bound, we define some operational numbers by probabilistic model as below
...
If we obtain the probability of each states $p[a_i]$, the expectation value of seat occupancy can be easily calculated.
Thus our problem for expectation of seat occupancies can be translated into how to set transition $p_k$ and how to derive $p[a_i]$ from $p_k$.

### Solution
Our steps to solve problem is

1. Set a transition matrix $T$ from seat number and grouping strategy.
