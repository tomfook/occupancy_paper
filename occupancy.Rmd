...
To make analysis easy, we only dispatch a vehicle when the seats is not enough for new arrivals.
...
Generally Markov chain state probability $P[s_t=k]$ is converge very fast to a stationary state.
...
By simulation, Option 3 looks the best in almost condition except for low $p_2$ at $p_1=0.1$.

The boundary between Option 1 and Option 3 should be on the root of
$$
F_{13}(p_1, p_2, p_3, p_4) = \mathrm{E[Occupancy|Option1] - E[Occupancy|Option3]} = 0.
$$

We can easily show the root is independent on $p_1$.
Thus the boundary must be the same to Fig.5 for any $p_1$.
